...
describe('API Request Tests', function () {
  ...
  it('Gets the current status of Ocean services for all networks', async () => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      assert(data[i].currentBlock === recentBlock, 'Missing field currentBlock')
      assert(data[i].market === State.Up, 'Missing field: market')
      assert(data[i].port === State.Up, 'Missing field: port')
      assert(data[i].aquarius.status === State.Up, 'Missing field: aquarius')
      assert(data[i].provider.status === State.Up, 'Missing field: provider')
      assert(data[i].subgraph.status === State.Up, 'Missing field: subgraph')
      assert(data[i].operator.status === State.Up, 'Missing field: operator')
      assert(data[i].dataFarming === State.Up, 'Missing field: dataFarming')
      assert(data[i].daoGrants === State.Up, 'Missing field: daoGrants')
      assert(data[i].lastUpdatedOn === date, 'Missing field: lastUpdatedOn')
    }
  })
})
